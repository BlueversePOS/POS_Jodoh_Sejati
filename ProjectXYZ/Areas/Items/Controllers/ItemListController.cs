...
namespace ProjectXYZ.Areas.Items.Controllers
{
    public class ItemListController : Controller
    {
        ...
        public SettingsRepo dtaccessSett = new SettingsRepo();
        ...
        public JsonResult GetTaxes(string Prefix)
        {
            ...
            try
            {
                ...
                DataTable ObjList = dtaccessSett.GetDataTaxes("");
                List<DataRow> rows = ObjList.Select().ToList();
                if (rows.Count() > 0 && !string.IsNullOrEmpty(Prefix))
                {
                    rows = ObjList.Select(string.Format("Tax_Name='{0}'", Prefix)).ToList();
                }
                var list = (from DataRow ro in rows
                            select new
                            {
        ...
        public JsonResult GetDetailItems(string param)
        {
            ...
            try
            {
                ...
                byte[] bytes = null;
                string _FileNamePathFoto = string.Empty;
                ...
                if (rows.Count > 0)
                {
                    if (!string.IsNullOrEmpty(rows[0]["Item_Image"].ToString().Trim()))
                    {
                        _FileNamePathFoto = Path.Combine(path, rows[0]["Item_Image"].ToString().Trim());
                        FileInfo fi = new FileInfo(_FileNamePathFoto);

                        using (FileStream fs = new FileStream(_FileNamePathFoto, FileMode.Open, FileAccess.Read))
                        {
                            // Create a byte array of file stream length
                            bytes = System.IO.File.ReadAllBytes(_FileNamePathFoto);
                        ...
                        FileContent = string.Format("data:{0};base64,{1}", ContentType, Convert.ToBase64String(bytes));
                        Size = bytes.Length;
        ...
        [AuthorizeActionFilterAttribute]
        public JsonResult UploadDoc(Dokumen model)
        {
            try
            {
                string path = Server.MapPath("~/Uploads/");
                ...
                string DOCID = model.DOCID.Trim();
                var filePaths = Directory.GetFiles(path, DOCID + "_*", SearchOption.TopDirectoryOnly);

                string DOC_FileName = string.Empty;
                string DOC_FileNamePath = string.Empty;
                string DOC_FilePath = string.Empty;

                if (model.DOCFILE != null)
                {
                    DOC_FileName = model.DOCFILE.FileName;
                    DOC_FileNamePath = string.Format("{0}_{1}{2}",
                        DOCID + "_DOC",
                        //Path.GetFileNameWithoutExtension(model.DOCFILE.FileName),
                        DateTime.Now.ToString("yyyyMMdd_HHmmss"),
                        Path.GetExtension(model.DOCFILE.FileName));
                    DOC_FilePath = Path.Combine(path, DOC_FileNamePath);
                    model.DOCFILE.SaveAs(DOC_FilePath);

                    byte[] uploadfile = new byte[model.DOCFILE.InputStream.Length];
                    long bytesRead = model.DOCFILE.InputStream.Read(uploadfile, 0, (int)model.DOCFILE.InputStream.Length);
                    model.DOCFILE.InputStream.Close();
                    string base64String = System.Convert.ToBase64String(uploadfile, 0, uploadfile.Length);

                    return Json(new { success = true, path = DOC_FileNamePath, file = base64String });
                }
                else
                {
