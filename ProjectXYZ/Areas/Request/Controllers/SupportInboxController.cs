...
using CMS.Areas.Request.Models;
using CMS.Models;
using System;
...
namespace CMS.Areas.Request.Controllers
{
    public class SupportInboxController : Controller
    {
        ...
        [Obsolete]
        public JsonResult GetSupportInboxList(SupportInbox model)
        {
            bool success = false;
            var email = Session["email"].ToString();
            try
            {
                DataTable ObjList = dtaccess.GetSupportInboxList(model);
                ...
                if (rows.Count > 0)
                {
                    if (!string.IsNullOrEmpty(rows[0]["DOCFILENAME"].ToString().Trim()))
                    {
                        _FileNamePath = Path.Combine(path, rows[0]["DOCFILEPATH"].ToString().Trim());
                        FileInfo fi = new FileInfo(_FileNamePath);

                        using (FileStream fs = new FileStream(_FileNamePath, FileMode.Open, FileAccess.Read))
                        {
                            // Create a byte array of file stream length
                            bytes = System.IO.File.ReadAllBytes(_FileNamePath);
                            //Read block of bytes from stream into the byte array
                            fs.Read(bytes, 0, System.Convert.ToInt32(fs.Length));
                            //Close the File Stream
                            fs.Close();
                        }
                        FileContentType = GetMimeType(fi.Extension);
                        FileContent = string.Format("data:{0};base64,{1}", FileContentType, Convert.ToBase64String(bytes));
                ...
                var list = (from DataRow ro in rows
                            select new
                            {
                                ...
                                PRIORITY_ID = ro["PRIORITY_ID"],
                                PRIORITY_DESC = ro["PRIORITY_DESC"],
                                CATEGORY_HELPID = ro["CATEGORY_HELPID"],
                                CATEGORY_HELPDESC = ro["CATEGORY_HELPDESC"],
                                SUBJECT_HELP = ro["SUBJECT_HELP"],
                                MESSAGE_HELP = ro["MESSAGE_HELP"],
                                DOCCODE = ro["DOCCODE"],
                                DOCFILENAME = ro["DOCFILENAME"],
                                DOCFILEPATH = string.IsNullOrEmpty(ro["DOCFILEPATH"].ToString().Trim()) ? "" : ro["DOCFILEPATH"].ToString().Trim(),
                                DOCFILE = ro["DOCFILE"],
                                DOCFILEDATE = ro["DOCFILEDATE"],

                                FileContentType = FileContentType,
                                FileContent = FileContent,
                                FileSize = FileSize,
                                FileDate = ro["DOCFILEDATE"],

                                STATUS = ro["STATUS"],
                                IP_ADDRESS = ro["IP_ADDRESS"],
                                CREATEDUSRID = ro["CREATEDUSRID"],
                                CREATEDDT = ro["CREATEDDT"],
                                MDFUSRID = ro["MDFUSRID"],
                                MODIFDT = ro["MODIFDT"],
                                FLAG_NAME = ro["FLAG_NAME"],
                                FLAG_ICON = ro["FLAG_ICON"],
                                FIRSTNAME = ro["FIRSTNAME"],
                                LASTNAME = ro["LASTNAME"],
                                STATUSTEXT = ro["STATUSTEXT"],
                                STATUSCOLOR = ro["STATUSCOLOR"]
                                //, HCMGROUPNAME = ro["HCMGROUPNAME"]
                            }).ToList();
        ...
        [AuthorizeActionFilterAttribute]
        [Obsolete]
        public ActionResult SupportInboxUpdate(SupportInbox model)
        {
            ...
            try
            {
                ...
                //{
                //    model.IPADDRESS = Session["IPADDRESS"].ToString() + "/" + Session["ADDRESS"].ToString();
                //}
                //else
                //{
                //    model.IPADDRESS = Session["IPADDRESS"].ToString();
                //}
                //model.BROWSER = Session["BROWSER"].ToString();

                model.IPADDRESS = "";
                model.BROWSER = "";

                dtResult = dtaccess.SupportInboxUpdate(model);
                if (dtResult.Rows.Count > 0)
                {
                    code = dtResult.Rows[0]["code"].ToString();
                    desc = dtResult.Rows[0]["desc"].ToString();

                    if (code.Trim() != "200") //error
                    {
        ...
        [AuthorizeActionFilterAttribute]
        [Obsolete]
        public JsonResult SupportInboxGetProgress(int DOCNM)
        {
            bool success = false;
            var email = Session["email"].ToString();
            try
            {
                DataTable ObjList = dtaccess.SupportInboxGetProgress(DOCNM);
                List<DataRow> rows = ObjList.Select().ToList();
                ...
                var list = (from DataRow ro in rows
                            select new
                            {
                                NO = i++,
                                DOCNM = ro["DOCNM"],
                                LNITMSEQ = ro["LNITMSEQ"],
                                VENDORID = ro["VENDORID"],
                                EMAILADDRESS = ro["EMAILADDRESS"],
                                STATUS_OLD = ro["STATUS_OLD"],
                                STATUS = ro["STATUS"],
                                EXPLANATION = ro["EXPLANATION"],
                                FIRSTNAME = ro["FIRSTNAME"],
                                LASTNAME = ro["LASTNAME"],
                                FULLNAME = string.Format("{0} {1}", ro["FIRSTNAME"], ro["LASTNAME"]).Trim(),
                                STATUSTEXTOLD = ro["STATUSTEXTOLD"],
                                STATUSCOLOROLD = ro["STATUSCOLOROLD"],
                                STATUSTEXT = ro["STATUSTEXT"],
                                STATUSCOLOR = ro["STATUSCOLOR"],
                                IP_ADDRESS = ro["IP_ADDRESS"],
                                CREATEDUSRID = ro["CREATEDUSRID"],
