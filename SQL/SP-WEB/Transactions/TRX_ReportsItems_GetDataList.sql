...
BEGIN
	BEGIN TRY
		...
		OUTER APPLY(
			select Item_Number, 0 LineItem_Option, SUM(Quantity) Quantity, SUM(Item_Price * Quantity) Gross_Sales, SUM((Item_Price * Quantity) - (ISNULL(Discount_Amount, 0) * Quantity)) Net_Sales,
