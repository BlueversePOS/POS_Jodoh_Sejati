create or alter proc [dbo].[Web_Items_SaveData]
(
	@UserID nvarchar(20),
...
BEGIN
	BEGIN TRY
		...
		BEGIN
			...
			Item_Shape=@Item_Shape, Item_Image=@Item_Image, Modified_User=@UserID, Modified_Date=@CurrDate
		...
		BEGIN
			...
			@Item_Shape, @Item_Image, @UserID, @CurrDate, '', ''
			FROM POS_Set_Site X
		END
		...
		SELECT @Item_Number, Item_Quantity, Item_Cost, Item_Composite, @UserID, @CurrDate, @UserID, @CurrDate
		...
		SELECT @Item_Number, ROW_NUMBER() OVER(ORDER BY Item_Number), Item_Quantity, Item_Cost, Item_Composite, @UserID, @CurrDate
		FROM @COMPTYPE --WHERE Item_Number=@Item_Number
		...
		OptimalStock=B.OptimalStock, Item_SKU=B.Item_SKU, Item_Barcode=B.Item_Barcode, Modified_User=@UserID, Modified_Date=@CurrDate
		...
		B.LowStock, B.OptimalStock, B.Item_SKU, B.Item_Barcode, @UserID, @CurrDate, @UserID, @CurrDate);
		...
		A.LowStock, A.OptimalStock, A.Item_SKU, A.Item_Barcode, @UserID, @CurrDate
